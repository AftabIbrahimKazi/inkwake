...
1. **Pseudo-classes/pseudo-elements CAN become utilities via variant prefixes (`hover:`, `focus:`, `placeholder:`, `group-hover:`, `peer-checked:`, …) — check the installed `CHANGELOG.md`/README `## Variants` section for the current list before assuming otherwise.** This reversed a wrong "permanently real CSS" claim once already (Strata added a full variant system in 1.9.0) — a version-pinned structural claim, not evergreen; re-verify after every bump. What variant prefixes still can't cover: arbitrary attribute selectors that aren't one of the framework's named states (e.g. `[aria-expanded="true"] .child`, project-defined `[data-active="true"]`), and reaching a *different* CSS variable than the base utility already uses (e.g. `hover:bg-primary` resolves to the same `--st-primary` as plain `bg-primary` — it can't reach a distinct `--st-primary-hover`).
