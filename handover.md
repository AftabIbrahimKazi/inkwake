...
- ~~`strata.components.js` is 259 bytes because `@strata-packages/*` are not installed~~ — **resolved 2026-09-02, not a defect.** CSS cost is the benchmark's primary parameter; JS cost is secondary and *expected* to vary since all three apps have different JS architectures by design (Bootstrap/Alpine ship one monolithic bundle always; Strata deliberately splits interactive components into standalone opt-in packages, each usable outside Strata too). `strata-app`'s hand-rolled mobile drawer instead of installing `@strata-packages/offcanvas` is not an unfair workaround — it's the intended framework-agnostic pattern. Leave as-is; don't install the packages for this benchmark.
